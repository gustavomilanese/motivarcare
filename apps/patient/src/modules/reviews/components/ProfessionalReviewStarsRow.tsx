...
export function ProfessionalReviewStarsRow(props: {
  ...
  return (
    <span
      className={`professional-review-stars-row professional-review-stars-row--${props.size ?? "md"}`}
      aria-label={`${displayRating.toFixed(1)} / 5`}
    >
      {[1, 2, 3, 4, 5].map((value) => (
        <span
          key={value}
          className={`professional-review-stars-row-star${value <= filledStars ? " filled" : ""}`}
          aria-hidden="true"
        >
          ★
        </span>
