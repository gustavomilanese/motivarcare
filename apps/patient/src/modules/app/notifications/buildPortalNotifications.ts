...
import type { ExercisePost } from "../../exercises/services/exercisesApi";
import type { ApiChatThread, Booking, Message, PatientAppState } from "../types";
import type { PortalNotificationItem, PortalNotificationKind } from "./portalNotificationTypes";
import {
  isNotificationDismissed,
  readPaymentFailureNotice,
  readSeenAssignedProfessionalId,
  readSeenExercisesPublishedAt
} from "./portalNotificationStorage";
...
function buildAccountNotifications(params: {
  ...
  if (assignedId) {
    const seenId = readSeenAssignedProfessionalId();
  ...
  const paymentNotice = readPaymentFailureNotice();
  if (paymentNotice && !isNotificationDismissed(paymentNotice.id)) {
    items.push({
      id: paymentNotice.id,
      kind: "payment-failed",
...
export function buildExerciseNotification(params: {
  ...
  const seenAt = readSeenExercisesPublishedAt();
...
export function buildPortalNotifications(params: {
  ...
  const exerciseItem = buildExerciseNotification({ language: params.language, exercises: params.exercises });
  if (exerciseItem) {
...
export function countUnreadNotifications(items: PortalNotificationItem[]): number {
